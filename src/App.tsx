...
export default function App() {
  ...
  // ── Streak ──
  const computeStreak=useCallback((plan:Plan)=>{ let s=0; const d=new Date(today); while(true){ const data=lsGet(dayKey(d)); if(!data) break; const ok=MEAL_ORDER.every(id=>isMealComplete(plan,id,data[id])); if(!ok) break; s++; d.setDate(d.getDate()-1); } return s; },[]);
  ...
  // ── Computed ──
  const mealDone=(id:string)=>isMealComplete(mealPlan,id,dayData[id]);
  ...
  const weekMealDone=(mData:any,id:string)=>mData&&isMealComplete(mealPlan,id,mData[id]);
